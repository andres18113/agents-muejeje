...
Receipt history carries an explicit completeness status and Codex must read it before drawing any conclusion from an empty list. `reviews/sc` is non-evidentiary and can never prove `complete`: only a completed authoritative `reviews/cs` sweep within configured recovery bounds can. Result-list bounds are separate from authoritative reasoning; a full list never stops the sweep or silently hides a newer unindexed receipt. The structured result exposes `authoritativeExhaustive` separately from `outputTruncated`, so a bounded caller result never implies a smaller authoritative namespace. `partial` means output or authoritative reasoning was truncated or unreadable, with `review_history_truncated` or `review_history_completeness_unproven` as applicable. `indeterminate` means discovery could not run at all and says nothing either way; it must never be read as "no prior review exists".

Receipt publication and custody release are reported separately, and Codex must not infer either from the other. `publication` is `not-attempted`, `cancelled-before-authority` (no receipt can ever appear), `authoritative-pending` (a receipt may still land, so custody is held), or `authoritative-settled` with its disposition. A retained slot reporting `review_receipt_publication_unquiesced` is uncertainty, not a permanent lock: while the root request remains active, the same live coordinator may still release it under the unchanged guarded rules once that exact publication settles; after root stop it remains for ordinary reconciliation. The durable record - not the synchronous `custodyState` in the returned outcome - is authoritative for current custody.
