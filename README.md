...
Those two Bash layers are not equivalent, and the difference matters. `permissions.deny` is evaluated by the Claude runtime that owns the tool call, so every prohibition statable as a rule is denied there, in each spelling Windows resolves for the same program (`rm`, `rm.exe`, `rm.cmd`, `rm.bat`, `rm.com`). The PreToolUse hook is an additional policy layer that adds the judgements a static rule cannot express: Git subcommand shape, package publication, and resolving the effective executable behind wrappers such as `command`, `builtin`, `env`, `timeout`, `nice`, `nohup`, `stdbuf` and `time`. The hook is a separate process with its own timeout, and a hook that times out or fails to start is not a denial - it is simply absent. It is therefore never the only barrier, and nothing here should be read as a claim that it is fail-closed.
