...
The runner uses `--restricted`, `--setting-sources ""`, `--strict-mcp-config`, and a private per-invocation settings file. The settings file contains only runtime policy - `permissions.deny` rules plus a Bash PreToolUse guard; it never contains the task body and is removed during terminal cleanup.

Those two Bash layers are not equivalent, and the difference matters. `permissions.deny` is evaluated by the Claude runtime that owns the tool call, so every prohibition statable as a rule is denied there, in each spelling Windows resolves for the same program (`rm`, `rm.exe`, `rm.cmd`, `rm.bat`, `rm.com`): prohibited commands, dangerous Git operations in first position (`git push`, `git commit`, `git -c`, and the rest), and the canonical package publication commands. The PreToolUse hook is an additional policy layer reserved strictly for the judgements a static prefix rule cannot express: resolving the effective executable behind wrappers such as `command`, `builtin`, `env`, `timeout`, `nice`, `nohup`, `stdbuf` and `time` (whose assignments, value flags, and positionals reorder the tokens), case variants, non-first-position matches, and arbitrary absolute paths. The hook is a separate process with its own timeout, and a hook that times out or fails to start is not a denial - it is simply absent. It is therefore never the only barrier for anything a rule can state, and nothing here should be read as a claim that it is fail-closed.
