...
Version 0.2.3 candidate. This local STDIO MCP server lets Codex act as the Lead while using fresh Claude Code specialist runs for bounded delegated work. Codex validates returned evidence, runs deterministic gates, and owns the final verdict.
...
`npm.cmd run diagnose` is a credential-free setup check. It reports the resolved version and availability of Node, npm, Git, Codex, and Claude, refuses a Node older than 20, and prints two verdicts: `repository-validation` (Node, npm, and Git, all that `npm.cmd run ci` needs) and `mcp-registration` (those plus Codex and Claude). It exits non-zero only when repository validation itself is blocked, because a missing Codex or Claude blocks registration, not validation. It never inspects authentication. The `claude-runtime` line is a readiness preflight, not a presence check: production launches every delegation with `--restricted`, which needs Claude Code 2.1.248 or newer, so a Claude below that floor is reported incompatible. Above the floor the binary is asked directly with `claude --restricted --help` - the flag plus `--help`, so it answers without doing any work - and only the binary rejecting that probe reports the capability missing. Help text corroborates in one direction only: mentioning the flag confirms it, but omitting it proves nothing, because help output is not a capability API.
...
Registering the MCP server additionally requires the Codex and Claude Code commands to be installed and authenticated. Repository tests do not. `install-codex.ps1` resolves and prints the exact `node`, `npm.cmd`, Codex, and Claude executables it will use, with their versions, and refuses a Node older than 20 before it registers anything. It likewise refuses a Claude Code older than 2.1.248 - the minimum that supports the `--restricted` flag production launches with - and warns without refusing when the Claude version cannot be parsed. It prefers the `.cmd`/`.exe` shims that actually execute on Windows and registers the resolved absolute Codex path rather than relying on `PATH` resolution at call time.
...
The rule behind both cases is one: a receipt binds exact commits, never a branch position. To review `B` against `A` is to freeze HEAD `B` and target `A` as OIDs, show `merge-base(A,B)..B`, and require the identical subject afterwards. A fast-forward preserves the reviewed commits, so the same delta keeps the same identity; anything that mints a new commit - merge, cherry-pick, squash, rebase, or conflict resolution - mints a new subject that needs its own bound review, even when the patch text is byte-identical.
...
The AFTER binding runs under the root request deadline, and a deadline observes work rather than proving it stopped. Receipt publication is therefore fenced across that boundary using the same rule Phase 5 applies to durable custody writes. Once the root request has stopped, no detached late-release mutation is started: the durable record remains for ordinary reconciliation.
...
If the bounded quiescence wait expires, nothing has been proven: the slot is retained rather than released, and the outcome reports `review_receipt_publication_unquiesced` alongside `coherent_admission_retained`. Retention there is uncertainty, not permanent ownership. While the root request remains active, the same still-live coordinator keeps watching that exact publication and, if it settles, may perform the same guarded release it would have performed synchronously, using the same terminal evidence and the same ownership and revision checks. Once the root request stops, settlement leaves the record for ordinary reconciliation instead. A record that has since moved to another owner, been archived, or been released is refused and stays fail-closed; the synchronously reported `custodyState` remains the snapshot it always was, and the durable record stays authoritative. Nothing else, and no other coordinator, may take that path.
...
An optional `target_ref` may be supplied to `delegate_agent` for `general-purpose`, `code-review`, or `security-review`. It must be fully qualified under `refs/heads/` or `refs/remotes/`. A review resolves the target independently during both collections; movement or deletion therefore changes the subject. No upstream, `origin/HEAD`, `origin/main`, or status-header target is inferred. A `general-purpose` worktree records its declared target so a later review inside that retained worktree can inherit it with explicit `worktree-metadata` provenance. A review of a clean committed worktree without a declared base has no subject: it still runs and returns its findings, but its binding is UNBOUND with reason `insufficient_review_scope` and no receipt is stored. The final committed review must therefore declare its base explicitly with `target_ref`; the reviewer is then automatically shown the complete `merge-base..HEAD` evidence for the declared `base..HEAD` range (in git-range terms `A..B` is `merge-base(A,B)..B`), derived from BEFORE's exact frozen commits - the captured HEAD OID and the captured target OID - never by re-resolving the mutable refs, so a ref that moves mid-collection cannot corrupt the bound subject. AFTER re-collects and requires the identical `changeSetId`, and the receipt binds that frozen evidence as well.
...
14. `changeSetId` does not include repository identity. Receipts are nevertheless stored beneath the repository's own SHA-256 coordination identity, and receipt provenance must match that scope before a stored receipt is used.
...
The runner uses `--restricted`, `--setting-sources ""`, `--strict-mcp-config`, and a private per-invocation settings file. `--restricted` is what constrains tool exposure for every delegation, and it needs Claude Code 2.1.248 or newer - the floor the installer refuses below and `diagnose` preflights against. The settings file contains only runtime policy - `permissions.deny` rules plus a Bash PreToolUse guard; it never contains the task body and is removed during terminal cleanup.

Those two Bash layers are not equivalent, and the difference matters. `permissions.deny` is the fail-closed gate: it is evaluated by the Claude runtime that owns the tool call whether the hook runs or not, so every prohibition statable as a rule is denied there - prohibited commands, dangerous Git operations (`git push`, `git commit`, `git -c`, and the rest), and the canonical package publication commands, each in every spelling the OS resolves (bare, `.exe`/`.cmd`/`.bat`/`.com`), behind wrappers at any nesting depth (`env`, `timeout 5 env`, absolute wrapper paths), and under absolute paths in both slash styles. A timed-out or failed PreToolUse hook does not block the tool call, so none of those spellings depends on it. The hook stays configured as defense in depth and for the judgements no rule shape can express: case variants (`GIT push`), non-first-position publication (`npm run publish`, a local script under the same authority as `npm test`), and quoted or composed commands, which it refuses wholesale. On POSIX systems a case-variant external command fails to resolve on its own, which narrows the hook-only residue to Windows. Nothing here claims the hook is fail-closed, and nothing here claims an OS sandbox: an allowed `node`, `python`, or `npm` child exercises its own authority once started.

If autonomous mutation needs a boundary stronger than a cooperative policy, that boundary has to come from outside this process: run the coordinator inside WSL2, a container, or a VM, or drive it through a constrained executor that owns the filesystem and network itself. This project does not provide one and does not claim one.

The honest limit of the whole Bash surface: it is a cooperative policy on the commands Claude asks to run, not an operating-system sandbox. Native Windows Bash is not sandboxed. A shell blocklist does not transitively constrain what an already-permitted `node`, `python`, or `npm run` child process then does - such a child can open sockets, delete files, and spawn anything the user account may spawn. The prohibitions above remove the direct, deterministically classifiable paths; they do not bound arbitrary code the agent is otherwise allowed to execute. The child environment is built from a compatibility allowlist instead of inheriting `process.env`, so ordinary secret-bearing variables are not passed by default. One invocation has an absolute execution deadline for useful work, a separate fixed proof-of-death deadline that begins when forced termination starts (including its durable transition), and a bounded housekeeping deadline for settings cleanup. A deadline winning a Promise race is bounded observation, not quiescence: it requests cancellation of a durable lifecycle mutation. A mutation still before its rename boundary cannot publish; a rename already issued remains serialized and quiesces before any later custody mutation or handoff. A cleanup timeout is reported separately and never discards already-proven close evidence. Immediately before the first prompt byte is written, the runner rechecks the absolute execution deadline; a clean close before delivery is a pre-ready failure even when it emitted startup stdout.

On Windows, process identity is `{pid, process StartTime UTC ticks}` obtained through `System.Diagnostics.Process.GetProcessById` and `Process.StartTime`; output is machine-readable invariant data, not localized text. PID existence alone is never a destructive or release decision. Forced timeout/overflow termination passes two independent gates before it may create a `taskkill /PID <pid> /T /F` helper. The durable gate comes first: the ownership record must already have published `TERMINATING`. If that publication fails, times out, is cancelled before its rename, or otherwise cannot be proven, no PID-based helper is created at all and only the exact in-memory `ChildProcess` handle is asked to die. The identity gate follows: a fresh recheck that the PID still has the stored start identity, where only `SAME_PROCESS` authorizes the command. The durable gate exists because a taskkill helper is a detached process: launching one while the record still said `ACTIVE` would let a crash strand a record that reconciliation may legitimately release while that helper was still running. The helper's PID-plus-start-time identity is captured at spawn and carried into the orphan record, so the same live coordinator can later observe that exact helper gone; but only that coordinator holds the launch report for a record that stayed `TERMINATING`, which is why such records never release automatically after that coordinator dies. The enforced invariant is therefore: a taskkill helper was launched only if durable `TERMINATING` was published first, which is what keeps the `ACTIVE` + coordinator-dead + Claude-dead release path safe. If taskkill errors, exits unsuccessfully, or hangs, supervision falls back to a request through the exact in-memory `ChildProcess` handle and waits the remaining proof-of-death grace. Once taskkill is launched, the synchronous handoff additionally requires the helper's own `close`: target-child `close` proves only the target, while helper `close` proves only that the destructive helper has quiesced. If either proof is absent at its bounded deadline, custody remains fail-closed; a later same-session reclaim may still proceed, but only on positive evidence - a never-launched report, a proven close, or every known helper identity observed gone - and never on the target's death alone. Dead, reused, or ambiguous identities are never sent to `taskkill`, and requesting termination alone never returns custody. Only `close` is terminal proof. Node's `exit` means the direct child ended while its stdio may still be open, which is exactly the case where a descendant still holds the inherited pipes, so `exit` is kept as a diagnostic observation and never returns write custody on its own. Initial PID-plus-start-time binding is race-hardened by refusing a query result once the exact child has already exited or closed, but it is not atomic or handle-bound; formal elimination requires native Windows process-handle identity/containment such as a future Job Object design. The honest guarantee is therefore: Phase 5 proves termination of the exact supervised Claude child and any launched destructive helper before returning custody; it does not prove that every escaped descendant is dead. A stronger transitive writer-authority guarantee requires Job Object or process-tree containment and remains future hardening. A dependency-free Node implementation does not provide reliable Job Object containment, so abrupt coordinator death may leave Claude descendants alive. Because `taskkill` itself accepts only a PID, Phase 5 also retains a narrow start-identity check/use race before taskkill; a Job Object or held native process handle would be needed for that stronger guarantee. Durable reconciliation detects and blocks a still-live recorded Claude process, but Phase 5 does not claim automatic process-tree cleanup after coordinator crash or cross-platform process identity. Fresh context and `--no-session-persistence` remain mandatory; there is no persistent parent, stage-persistent context, nested delegation, automatic integration, or parallel writer support.
...
Layer 1: Outer Codex MCP Client Tool Timeout (tool_timeout_sec = 3600)
    ↓
Layer 2: Root MCP Response Envelope (Useful-Work + Settlement Budget = 2415s)
    ↓
...
2. **Root MCP response envelope (`2415s` = 40.25 minutes)**:
   `delegate_agent` creates one root request context at entry: `{ deadlineAt, abortSignal, now }`. Its response envelope is the effective useful-work timeout plus a strictly bounded **synchronous settlement budget** of `615s` (10.25 minutes), currently `2415s` (40.25 minutes) at the supported 30-minute maximum. CWD/workspace and repository identity resolution, admission, worktree preparation, target and ChangeSet observation, history recovery, artifact loading, runner supervision, AFTER binding, publication, and settlement all consume this one envelope. Useful work is clipped to leave the settlement reserve; after cancellation or the root deadline, no new evidence observation, Git/worktree action, admission, receipt publication, or detached custody release begins. A root stop returns the MCP outcome and retains any unsettled custody for reconciliation. This is a response-settlement bound under normal event-loop scheduling, not a claim that an already-issued kernel filesystem operation or escaped process tree has been preempted; those operations receive cancellation or fixed supervision where available, and their durable disposition stays fail-closed until observed. This budget accounts for:
...
3. **Specialist useful-work execution cap (`1800s` = 30 minutes)**:
   This is the maximum configured useful-work timeout; the effective runner timeout is clipped further by remaining root-envelope time and the settlement reserve. In `src/agent-registry.mjs`, registered profile execution timeouts are:
...
- Or if the client disconnects, cancellation signals must be propagated to terminate the child and cancel the publication fence; custody whose terminal disposition is no longer observed remains retained for reconciliation rather than being released by a post-cancellation mutation.

To provide configured headroom against client abandonment during normal execution, `src/timeout-policy.mjs` provides a machine-checked invariant:
