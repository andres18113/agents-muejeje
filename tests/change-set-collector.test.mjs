...
function statusStream(records, { branchOid = HEAD, branchHead = "main" } = {}) {
  const fields = ["# branch.oid " + branchOid, "# branch.head " + branchHead, ...records];
  return Buffer.concat(fields.map((field) => Buffer.concat([Buffer.from(field, "utf8"), Buffer.of(0)])));
}

function ordinary(xy, pathText, { sub = "N...", modeWorktree = "100644", oidHead = OID_A, oidIndex = OID_B } = {}) {
  return "1 " + xy + " " + sub + " 100644 100644 " + modeWorktree + " " + oidHead + " " + oidIndex + " " + pathText;
}
